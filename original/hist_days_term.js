/* hist_days_term.js
   D3 histogram of "days/term" from a CSV.
   ...
   By default it loads "data.csv". To override:
     window.HIST_DAYS_TERM = { csvUrl: "myfile.csv", selector: "#chart", bins: 30 };
*/
...
(function () {
  const cfg = Object.assign(
    {
      ...
      selector: "#tenure",
      column: "days/term",
      bins: 10, // number of bins; you can also set to e.g. d3.thresholdFreedmanDiaconis
      width: 500,
      height: 200,
  ...
  function parseNumber(v) {
    ...
    if (!s) return null;
    // remove commas and any stray non-number chars except . and -
  ...
  d3.csv(cfg.csvUrl)
    .then((rows) => {
      const values = rows
        .map((d) => parseNumber(d[cfg.column]))
        .filter((d) => d != null);
      ...
      // If all values equal, pad domain a bit so bins render
      const xDomain =
      ...
      g.append("g")
        ...
        .call(d3.axisBottom(x).tickValues([0,10*365,20*365,30*365]).tickFormat(function(d){return d/365}))
        .call((axisG) =>
          axisG
            .selectAll("text")
            .attr("font-family", "system-ui, -apple-system, Segoe UI, Roboto, sans-serif")
      ...
      bar
        ...
        .text(
          (d) =>
            `${cfg.column}: ${d.x0?.toFixed?.(0) ?? d.x0}–${d.x1?.toFixed?.(0) ?? d.x1}\nCount: ${d.length}`
        );
    ...
    .catch((err) => {
      ...
      g.append("text")
        ...
        .attr("text-anchor", "middle")
        .attr("font-family", "system-ui, -apple-system, Segoe UI, Roboto, sans-serif")
        .text(`Failed to load CSV: ${cfg.csvUrl}`);
    });
})();
