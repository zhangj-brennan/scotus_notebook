...
export function countSurvivors(data, thresholdYears) {
  const calculationData = getCalculationData(data);
  const survived = calculationData.filter(d => d.tenureYears >= thresholdYears).length;
  const total = calculationData.length;
  return { survived, total, rate: total ? survived / total : 0 };
}

export function splitCounts(data, splitDate, thresholdYears) {
  const calculationData = getCalculationData(data);
  const left = calculationData.filter(d => d.startDate < splitDate);
  const right = calculationData.filter(d => d.startDate >= splitDate);
...
const x = d3.scaleTime()
  ...
  renderDots(data) {
    ...
    this.g.dots.selectAll("circle")
      ...
      .join("circle")
      .attr("class", d => {
        if (!d.isCurrent) return "dot";
        return d.includeInCalculations === false ? "dot current current-outline" : "dot current";
      })
...
this.renderQuadrants(counts, splitDate, threshold);
  ...
  renderScene(sceneName, sceneConfig, data) {
    ...
    if (sceneName === "scene1") {
      const calculationData = getCalculationData(data);
      const median = d3.median(calculationData, d => d.tenureYears) ?? 0;
